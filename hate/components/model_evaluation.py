...
from hate.configuration.gcloud_syncer import GCloudSync ## beacuse to check whether this model exist in GCp cloud
...
class ModelEvaluation:
    def __init__(self,model_evaluation_config:ModelEvaluationConfig,
        ...
        """
        param model_evaluation_config : config for model evaluation
        model=model_trainer
        data transformation artifacts

        : param model_trainer_artifacts: output reference of model trainer artifacts stage
        """

        self.model_evaluation_config=model_evaluation_config
        self.model_trainer_artifacts=model_trainer_artifacts
        self.data_transformation_artifacts=data_transformation_artifacts
        self.gcloud=GCloudSync() # beacuse this will give 2 obj


    def get_best_model_from_gcloud(self) ->str:
        """
        return: Fetch best model from gcloud storage and store inside best model direct path

        """

        try:
            logging.info("Entered the get_best_model_from_gcloud method of Model Eval class")

            os.makedirs(self.model_evaluation_config.BEST_MODEL_DIR_PATH,exist_ok=True)
                # this will download the best model in local machine and let do prediction
            ...
            logging.info("EXited the get_best_model_from_gcloud method of Model Eval class")
    ...
    def evaluate(self):
        ...
        try:
            logging.info("Entering into the evaluate func of model eval class")
    ...
    def initiate_model_evaluation(self)->ModelEvaluationArtifacts:
        ...
        logging.info("Initiate Model Eval")
        try:
            ...
            logging.info("check is best model present in the gcloud or not")
            if os.path.isfile(best_model_path) is False:
                is_model_accepted=True

                logging.info("g cloud model is false and currently trained model accpetd is true")

            else:
                logging.info("Load the model fetched from gcloud strage")
                best_model=keras.models.load_model(best_model_path)
                best_model_accuracy=self.evaluate()

                logging.info("Comaparing loss between best_model_loss and trained_model_loss")

                if best_model_accuracy > trained_model_accuracy:
                    is_model_accepted=True
                    logging.info("Trained model not accpeted")
            ...
            logging.info("Returning the ModelEval Artifacts")
