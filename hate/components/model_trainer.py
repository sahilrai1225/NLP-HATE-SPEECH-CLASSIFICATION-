...
class ModelTrainer:
    ...
    def splitting_data(self,csv_path):
        try:
            logging.info("Entered the splitting_data func")
            ...
            logging.info("Applying train_test_split on data")
            x_train,x_test,y_train,y_test=train_test_split(x,y,test_size=0.3,random_state=RANDOM_STATE)
            print(len(x_train),len(y_train))
            print(len(x_test),len(y_test))
            print(type(x_test),type(y_test))
            logging.info("Exited the splitting data func")
    ...
    def tokenizing(self,x_train):
        try:
            ...
            logging.info(f"converting test to sequnces :{sequences}")
            sequences_matrix=pad_sequences(sequences,maxlen=self.model_trainer_config.MAX_LEN)
            logging.info(f"The sequnence matrix is {sequences_matrix}")
    ...
    def initiate_model_trainer(self,) ->ModelTrainerArtifacts:
        ...
        Desc: This fun initiate modeltrainer steps

        output : Return model trainer artifacts
        on Failure: write an exception and then raise an excpetion
        
        """

        try:
            logging.info("entered the initiate_model_trainer funct")
            ...
            logging.info(f"Xtest shape is : {x_test.shape}")
